...
import { TraceEventRecord } from "@/types/conversation";

interface TracePanelProps {
  trace: TraceEventRecord | null;
  isOpen: boolean;
  onToggle: () => void;
  compact?: boolean;
}
...
export function TracePanel({ trace, isOpen, onToggle, compact = false }: TracePanelProps) {
  const confidencePercent = trace?.confidence != null ? `${Math.round(trace.confidence * 100)}%` : "-";
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-200/80 px-3 py-2.5 dark:border-slate-800/80">
        <div>
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-500">Routing, tools, and fallback outcomes</p>
        </div>
        <button
          type="button"
          onClick={onToggle}
          ...
          >
            {trace ? (
              <>
                ...
                <TraceRow label="Routing mode" value={trace.routingMode} icon={<Route size={14} />} />
                <TraceRow label="Confidence" value={confidencePercent} icon={<AlertTriangle size={14} />} />
                <TraceRow label="Latency" value={`${trace.latency.overallMs} ms`} icon={<Clock3 size={14} />} />
                ...
                <div className="rounded-[14px] border border-slate-300 bg-white/80 p-2.5 text-xs leading-5 text-slate-700 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-300">
                  ...
                  <p>{trace.routingReason}</p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    <StatusBadge label={trace.routingMode === "auto" ? "Auto route" : "Manual route"} tone="info" />
                    <StatusBadge
                      label={trace.fallbackState ? `Fallback: ${trace.fallbackState}` : "No fallback"}
                      tone={trace.fallbackState ? "warning" : "success"}
                    />
                  </div>
                </div>

                <TraceList label="Sources used" values={trace.sourcesUsed} />
                ...
                <div className="rounded-[14px] border border-slate-300 bg-white/80 p-2.5 dark:border-slate-800 dark:bg-slate-900/70">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {trace.toolCalls.length > 0 ? (
                      trace.toolCalls.map((tool) => <StatusBadge key={tool} label={tool} tone="neutral" />)
                    ) : (
                      <p className="text-xs text-slate-500 dark:text-slate-500">No tools recorded.</p>
                    )}
                  </div>
                </div>
                ...
                {trace.errors.length > 0 && (
                  <div className="rounded-[14px] border border-rose-300/60 bg-rose-100/70 p-2.5 text-xs text-rose-700 dark:border-rose-500/35 dark:bg-rose-500/10 dark:text-rose-200">
                    <p className="mb-1 font-semibold">Fallback / errors</p>
              ...
              <div className="rounded-[14px] border border-dashed border-slate-300 p-3 text-xs leading-5 text-slate-500 dark:border-slate-700 dark:text-slate-400">
                No trace yet. Send a message to inspect routing reason, latency, tools, and fallback state.
              </div>
            )}
          </motion.div>
        )}
